...
@section('title', 'تعديل منتج - لوحة التحكم')

@section('content')
<div class="page-header-bar">
    <h1 class="page-title">تعديل منتج</h1>
</div>
...
<div class="product-form-container">
    ...
    <form action="{{ route('customer.products.update', $product) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="product-form-grid">
            <div class="form-section">
                ...
                <div class="form-group-new">
                    ...
                    <div class="price-input-group">
                        ...
                        <select name="currency" id="currency" class="currency-select">
                            <option value="IQD" {{ old('currency', $product->currency) == 'IQD' ? 'selected' : '' }}>د.ع</option>
                            <option value="USD" {{ old('currency', $product->currency) == 'USD' ? 'selected' : '' }}>$</option>
                        </select>
                    </div>
                    @error('price')
                        <span class="invalid-feedback">{{ $message }}</span>
                ...
                <div class="form-group-new">
                    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
                        <input type="checkbox" name="manage_stock" value="1" {{ old('manage_stock', $product->manage_stock ?? true) ? 'checked' : '' }} id="manage_stock">
                        تتبع المخزون
                    </label>
                ...
                <div class="form-group-new">
                    <label for="sell_unit">وحدة البيع</label>
                        ...
                        @foreach($sellUnits as $unitOption)
                            <option value="{{ $unitOption }}" {{ old('sell_unit', $product->sell_unit) == $unitOption ? 'selected' : '' }}>
                                {{ $unitOption }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="form-section">
                            ...
                            @foreach($product->images as $image)
                                <div class="uploaded-image">
                                    ...
                                    <button type="button"
                                        ...
                                        onclick="if(confirm('حذف هذه الصورة؟')) document.getElementById('delete-image-form-{{ $image->id }}').submit()">
                                        ×
                                    </button>
                                </div>
                            @endforeach
                ...
                <div class="form-group-new">
                    ...
                    <input type="number" id="conversion_factor" name="conversion_factor" value="{{ old('conversion_factor', $product->conversion_factor) }}" step="0.0001">
                ...
                <div class="form-group-new">
                    ...
                    <input type="url" id="facebook_post_url" name="facebook_post_url" value="{{ old('facebook_post_url', $product->facebook_post_url) }}" placeholder="أدخل الرابط أو اختر من الصفحة...">
                ...
                <div class="form-group-new mt-4">
                    ...
                    <input type="url" id="instagram_post_url" name="instagram_post_url" value="{{ old('instagram_post_url', $product->instagram_post_url) }}" placeholder="أدخل الرابط أو اختر من الحساب...">
...
@if($product->images->count() > 0)
    @foreach($product->images as $image)
        <form id="delete-image-form-{{ $image->id }}" action="{{ route('customer.products.images.delete', $image) }}" method="POST" style="display: none;">
            @csrf
            @method('DELETE')
        </form>
    @endforeach
...
function removeImage(index) {
    // Note: Simplified preview removal; actual FileList manipulation would need DataTransfer.
    const container = document.getElementById('preview-container');
